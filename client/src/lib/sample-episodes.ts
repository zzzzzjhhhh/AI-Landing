...
import clothingPressureV4 from "../../../public/rerun/episodes/20260911_170529/visual-pressure-v4-reference.json";
...
export const sampleEpisodes: readonly SampleEpisode[] = [
  {
    ...
    flexionNote: "Stereo keypoints use provisional ACE raw tracking; 3D flexion retains the original pose. Pressure uses the same WebHand point-overlay renderer, palette, height scale and temporal interpolation as 165650, over the original gray hand. V4 contact labels are adapted as connected finger spans, not one peak per joint. Colors are display estimates, not measured force; uncolored regions may be unknown or no-contact. Movement holds the last pose during tracking gaps.",
